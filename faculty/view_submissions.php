...
if (isset($_GET['studentId'])) {
    ...
    $query = "SELECT * FROM micro_project WHERE username = ? AND evaluation_status <> 'evaluated'";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>alert('Already Reviewed'); window.location.href='project_submissions.php';</script>";
        exit;
    }
}

// Check if the record is already accepted
if ($projectDetails['mentor_status'] === 'Accepted') {
    echo "<script>alert('This project has already been accepted. Cannot update.'); window.location.href='project_submissions.php';</script>";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   // $dateOfCertificate = $_POST['date_of_certificate'];
    ...
    $updateQuery = "UPDATE micro_project SET 
                    ...
                    $updateStmt->bind_param('ssssssdsss', $usedTool, $projectStatus, $durationInWeeks, $validCertificate, $presented, $recommended, $formattedDate, $marks, $projectDetails['mentor_status'], $username);
   ...
   /* $gradeUpdateQuery = "UPDATE micro_project SET grade = ? WHERE username = ?";
    ...
    if ($projectStatus == 'Completed' && $validCertificate == 'Yes' && $presented == 'Yes' && $usedTool=='Yes' && $durationInWeeks>4 && $recommended=='Yes') {
        $acceptance = 'Accepted';
    } else {
        $acceptance = 'Rejected';
    }

    $acceptanceUpdateQuery = "UPDATE micro_project SET mentor_status = ? WHERE username = ?";
    ...
    echo "<script>alert('Evaluation has been submitted successfully.'); window.location.href='project_submissions.php';</script>";
    exit;
}
