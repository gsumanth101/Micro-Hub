            ...
            if (move_uploaded_file($tempname, $destination)) {
                if (file_exists($destination)) {
                    ...
                    for ($i = 1; $i < $sheetCount; $i++) {
                        $stmt = $conn->prepare("INSERT INTO external_events (id, name, date, college_name, brochure, website) VALUES (?, ?, ?,? , ?,?)");
                        $stmt->bind_param("ssssss", $spreadSheetAry[$i][0], $spreadSheetAry[$i][1], $spreadSheetAry[$i][2], $spreadSheetAry[$i][3], $spreadSheetAry[$i][4], $spreadSheetAry[$i][5]);
                        $stmt->execute();
                    }
