...
function exportToExcel($data, $filename) {
    ...
    $worksheet->setCellValue('F1', 'Section');
    $worksheet->setCellValue('G1', 'Year');
    $worksheet->setCellValue('H1', 'Marks');
    ...
    foreach ($data as $row) {
        ...
        $worksheet->setCellValue('F' . $rowNumber, $row['section']);
        $worksheet->setCellValue('G' . $rowNumber, $row['year']);
        $worksheet->setCellValue('H' . $rowNumber, $row['marks']);
        $worksheet->setCellValue('I' . $rowNumber, $row['acceptance_status']);
...
if (isset($_POST['download'])) {
    ...
    $export_sql = "SELECT * FROM micro_project WHERE acceptance_status='Rejected' $search_condition_sql";
    ...
    $excelFileName = 'micro_project_Rejected.xlsx';
